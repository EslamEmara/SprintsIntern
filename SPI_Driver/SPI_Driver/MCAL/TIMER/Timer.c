...
void Timer_init(ST_TIMER_config_t config)
{
	...
	adjustPrescalarT1(PRESCALAR0_64);
}
...
uint8_t Timer_Delay(float desired,ST_TIMER_config_t config)
{
	...
	if (config.timer == TIMER1)
	{
		uint8_t i = 0;
		uint8_t found = 0;
		EN_PRESCALAR0_t prescalar = PRESCALAR0_64;
		/*
		for(i=0;i<5;i++){
			if(((uint16_t)(config.freq/PRESCALARS_0[i]) < (65536.0/desired)) && ((config.freq/PRESCALARS_0[i]) > 0.0)){				
				prescalar = PRESCALARS_0[i];
				found = 1;
				break;
			}
